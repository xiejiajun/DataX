...
public class EmrfsWriter extends Writer {
    public static class Job extends Writer.Job {
        ...
        private EmrfsHelper emrfsHelper = null;
        ...
        public void init() {
            ...
            //创建textfile存储
            emrfsHelper = new EmrfsHelper();

            emrfsHelper.getFileSystem(defaultFS, this.writerSliceConfig);
        }

        private void validateParameter() {
            String bucket = this.writerSliceConfig.getNecessaryValue(Key.S3_BUCKET, EmrfsWriterErrorCode.REQUIRED_VALUE);
            this.defaultFS = String.format("s3://%s/",bucket);
            //fileType check
            this.fileType = this.writerSliceConfig.getNecessaryValue(Key.FILE_TYPE, EmrfsWriterErrorCode.REQUIRED_VALUE);
            if( !fileType.equalsIgnoreCase("ORC") && !fileType.equalsIgnoreCase("TEXT")){
                String message = "EmrfsWriter插件目前只支持ORC和TEXT两种格式的文件,请将filetype选项的值配置为ORC或者TEXT";
                throw DataXException.asDataXException(EmrfsWriterErrorCode.ILLEGAL_VALUE, message);
            }
            //path
            this.path = this.writerSliceConfig.getNecessaryValue(Key.PATH, EmrfsWriterErrorCode.REQUIRED_VALUE);
            if(!path.startsWith("/")){
            ...
            //fileName
            this.fileName = this.writerSliceConfig.getNecessaryValue(Key.FILE_NAME, EmrfsWriterErrorCode.REQUIRED_VALUE);
            //columns check
            this.columns = this.writerSliceConfig.getListConfiguration(Key.COLUMN);
            if (null == columns || columns.size() == 0) {
                throw DataXException.asDataXException(EmrfsWriterErrorCode.REQUIRED_VALUE, "您需要指定 columns");
            }else{
                for (Configuration eachColumnConf : columns) {
                    eachColumnConf.getNecessaryValue(Key.NAME, EmrfsWriterErrorCode.COLUMN_REQUIRED_VALUE);
                    eachColumnConf.getNecessaryValue(Key.TYPE, EmrfsWriterErrorCode.COLUMN_REQUIRED_VALUE);
                }
            }
            //writeMode check
            this.writeMode = this.writerSliceConfig.getNecessaryValue(Key.WRITE_MODE, EmrfsWriterErrorCode.REQUIRED_VALUE);
            ...
            if(null == fieldDelimiter){
                throw DataXException.asDataXException(EmrfsWriterErrorCode.REQUIRED_VALUE,
        ...
        public void prepare() {
            //若路径已经存在，检查path是否是目录
            if(emrfsHelper.isPathexists(path)){
                if(!emrfsHelper.isPathDir(path)){
                    throw DataXException.asDataXException(EmrfsWriterErrorCode.ILLEGAL_VALUE,
                            String.format("您配置的path: [%s] 不是一个合法的目录, 请您注意文件重名, 不合法目录名等情况.",
                                    path));
                }
                //根据writeMode对目录下文件进行处理
                Path[] existFilePaths = emrfsHelper.emrfsDirList(path,fileName);
                ...
                if ("overwrite".equals(writeMode) && isExistFile ) {
                 LOG.info(String.format("由于您配置了writeMode overwrite, 开始清理 [%s] 下面以 [%s] 开头的内容",
                 path, fileName));
                 emrfsHelper.deleteFiles(existFilePaths);
        ...
        @Override
        public void post() {
            emrfsHelper.renameFile(tmpFiles, endFiles);
        }

        @Override
        public void destroy() {
            emrfsHelper.closeFileSystem();
        }
        ...
        public List<Configuration> split(int mandatoryNumber) {
            ...
            //获取该路径下的所有已有文件列表
            if(emrfsHelper.isPathexists(path)){
                allFiles.addAll(Arrays.asList(emrfsHelper.emrfsDirList(path)));
            }
        ...
        private String buildTmpFilePath(String userPath) {
            ...
            while(emrfsHelper.isPathexists(tmpFilePath)){
                tmpSuffix = UUID.randomUUID().toString().replace('-', '_');
                if (!isEndWithSeparator) {
                    tmpFilePath = String.format("%s__%s%s", userPath, tmpSuffix, IOUtils.DIR_SEPARATOR);
                }else if("/".equals(userPath)){
    ...
    public static class Task extends Writer.Task {
        ...
        public void init() {
            ...
            String bucket = this.writerSliceConfig.getString(Key.S3_BUCKET);
            this.defaultFS = String.format("s3://%s/",bucket);
            this.fileType = this.writerSliceConfig.getString(Key.FILE_TYPE);
            //得到的已经是绝对路径，eg：s3://bucket/user/hive/warehouse/writer.db/text/test.textfile
            this.fileName = this.writerSliceConfig.getString(Key.FILE_NAME);

            emrfsHelper = new EmrfsHelper();
            emrfsHelper.getFileSystem(defaultFS, writerSliceConfig);
        }
        ...
        public void startWrite(RecordReceiver lineReceiver) {
            ...
            if(fileType.equalsIgnoreCase("TEXT")){
                //写TEXT FILE
                emrfsHelper.textFileStartWrite(lineReceiver,this.writerSliceConfig, this.fileName,
                        this.getTaskPluginCollector());
            }else if(fileType.equalsIgnoreCase("ORC")){
                //写ORC FILE
                emrfsHelper.orcFileStartWrite(lineReceiver,this.writerSliceConfig, this.fileName,
                        this.getTaskPluginCollector());
            }
